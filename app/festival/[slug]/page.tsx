...
export default function FestivalPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        const prodData = await productService.getProductsByFestival(slug);
        if (active) {
  ...
  const festivalTitle = slug.replace(/-/g, ' ');
  const festivalDesc = `Celebrate the season with our premium ${festivalTitle} collections and limited-time savings.`;
    ...
    <div className="min-h-screen bg-[#fcfbf9] py-8 sm:py-12">
      ...
      {productsList.length > 0 ? (
        ...
        <NoProductsFound
          ...
          description={`We couldn't find any products specifically listed for the ${festivalTitle} event.`}
        />
      )}
    </div>
  );
