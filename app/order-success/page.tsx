...
const ScratchCard = dynamic(() => import("@/components/ui/ScratchCard"), {
  ssr: false,
  loading: () => (
    <div className="h-40 bg-gray-50 rounded-2xl animate-pulse flex items-center justify-center text-xs text-gray-400">
      Loading your gift...
    </div>
  ),
});
...
export default function OrderSuccessPage() {
    ...
    async function loadCardDetails() {
      try {
        ...
        if (numAmount >= minLimit) {
          ...
          if (
            userCardsData.success &&
            userCardsData.cards &&
            userCardsData.cards.length > 0
          ) {
            // Find unrevealed card or latest assigned card
            const match =
              userCardsData.cards.find((c: UserScratchCard) => !c.is_claimed) ||
              userCardsData.cards[0];
  ...
  const getRewardDisplay = () => {
    ...
    if (userCard.reward_type === "percentage_discount")
            ...
            {amount >= minOrderThreshold && userCard ? (
              <div>
                <div className="flex items-center justify-center gap-2 mb-3">
                  ...
                  <h3 className="text-xs font-bold uppercase tracking-widest text-[#1a1a1a]">
                    You Unlocked A Scratch Card Gift!
                  </h3>
                </div>
                <ScratchCard
              ...
              <div className="p-4 bg-amber-50/80 border border-amber-200/70 rounded-2xl text-left space-y-1">
                ...
                <p className="text-[11px] text-amber-800 leading-relaxed">
                  Spend <span className="font-bold">₹{minOrderThreshold}</span>{" "}
                  or more on your next order to automatically unlock an
                  exclusive Scratch Card reward!
                </p>
              </div>
            ) : null}
          </div>
        ...
        <div className="space-y-4 text-left border-t border-gray-100 pt-6 mb-8 text-xs text-[#6b5b4b]">
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p>
                Your order will be shipped within 24-48 hours and arrive in 3-5
                business days.
              </p>
            </div>
          </div>
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p>
                A shipping confirmation email with tracking details will be sent
                shortly.
              </p>
            </div>
          </div>
