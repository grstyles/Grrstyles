...
export default function ProductDetailsClient({
  ...
  let shirtSizes: { size: string; stock: number; type: string }[] = [];
  let pantSizes: { size: string; stock: number; type: string }[] = [];
  let genericSizes: { size: string; stock: number; type: string }[] = [];

  if (isCombo) {
    const configuredShirt = allProductSizes.filter(s => STANDARD_SHIRT_SIZES.includes(s) || product?.shirtStock?.[s] !== undefined);
    ...
    const configuredPant = allProductSizes.filter(s => STANDARD_PANT_SIZES.includes(s) || product?.pantStock?.[s] !== undefined);
  ...
  const hasGeneric = !isCombo && genericSizes.length > 0;
  ...
  useEffect(() => {
    if (product) {
      ...
      } else if (product.imageColors && product.imageColors.length > 0) {
        setSelectedColorState(product.imageColors[0].color_name);
      }
    }
  }, [product]);
  ...
  useEffect(() => {
    ...
    async function loadData() {
      if (!slug || slug === product?.slug) return;
      setLoading(true);
      try {
  ...
  const availableStock = useMemo(() => {
    ...
    if (hasGeneric) {
      if (selectedSize) {
        const sStock = product.shirtStock?.[selectedSize] ?? product.pantStock?.[selectedSize] ?? product.shoeStock?.[selectedSize];
        if (sStock !== undefined) return Number(sStock);
      }
      return Number(product.overallStock ?? product.stockCount ?? 0);
    }
    return Number(product.overallStock ?? product.stockCount ?? 99);
  }, [product, isCombo, hasGeneric, selectedShirtSize, selectedPantSize, selectedSize]);
  ...
  const handleAddToCart = () => {
    ...
    const shirtSizeVal = isCombo ? (selectedShirtSize || undefined) : (isShirt ? selectedSize || undefined : undefined);
    const pantSizeVal = isCombo ? (selectedPantSize || undefined) : (isPant ? selectedSize || undefined : undefined);
    const shoeSizeVal = isShoe ? (selectedSize || undefined) : undefined;
    ...
    const isCouponEnabled = product.couponApplicable !== false && 
  ...
  const handleBuyNow = () => {
    ...
    dispatch(
      setDirectCheckoutItem({
        ...
        quantity: quantity,
        size: finalSize,
        shirtSize: isCombo ? (selectedShirtSize || undefined) : (isShirt ? selectedSize || undefined : undefined),
        pantSize: isCombo ? (selectedPantSize || undefined) : (isPant ? selectedSize || undefined : undefined),
        shoeSize: isShoe ? (selectedSize || undefined) : undefined,
  ...
  const mainImageSrc = visibleImages[activeImageIndex]?.url || visibleImages[0]?.url || '/images/placeholder.jpg';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div className="flex flex-col">
            ...
            {/* Coupon Badge/Tag */}
            <ProductCouponBadge product={product} />
            ...
            <div className="mb-5">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium text-[#1a1a1a]">
                  {isCombo ? 'Sizes' : (isPant ? 'Waist Size' : isShoe ? 'Shoe Size' : 'Size')}
                  {sizeWarning && <span className="text-red-500 text-xs ml-2">* Required</span>}
                </span>
              </div>
              
              <div className="space-y-4">
                  {isCombo && shirtSizes.length > 0 && (
                    <div>
                      <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 block">Select Shirt Size</span>
                  ...
                  {isCombo && pantSizes.length > 0 && (
                    <div>
                      <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 block">Select Pant Size</span>
                  ...
                  {(!isCombo && !hasGeneric && shirtSizes.length === 0 && pantSizes.length === 0) && (
                    ...
                    return (
                      ...
                      >
                        <div className={`relative w-[72px] h-[90px] sm:w-[80px] sm:h-[100px] rounded-lg overflow-hidden bg-[#f5f0eb] transition-all duration-300 ${
                          ...
                          <Image
                            src={imgObj.image_url || '/images/placeholder.jpg'}
                            alt={colorName}
                            fill
                            sizes="80px"
            ...
            <div className="mb-5">
              ...
              <div className="flex items-center border border-gray-200 rounded-xl w-36">
                <button
                  ...
                  className="px-4 py-3 hover:bg-gray-100 text-gray-400 hover:text-black transition-colors rounded-l-xl disabled:opacity-40 disabled:cursor-not-allowed"
                  disabled={quantity <= 1 || availableStock === 0}
                  aria-label="Decrease quantity"
                >
                  <Minus size={16} />
                </button>
                <span className="flex-1 text-center text-sm font-medium">{availableStock === 0 ? 0 : quantity}</span>
                <button
                  onClick={() => {
                    if (quantity >= availableStock) {
                  ...
                  className="px-4 py-3 hover:bg-gray-100 text-gray-400 hover:text-black transition-colors rounded-r-xl disabled:opacity-40 disabled:cursor-not-allowed"
        ...
        <div className="fixed inset-0 bg-black/95 z-[100] flex flex-col justify-between p-4 sm:p-6 select-none">
          ...
          <div className="flex-1 flex items-center justify-center relative max-h-[80vh]">
            ...
            <div className="relative w-full h-full max-w-4xl aspect-[3/4]">
              <Image
                src={visibleImages[activeImageIndex]?.url || '/images/placeholder.jpg'}
                alt={product.title || 'Product Image'}
                fill
