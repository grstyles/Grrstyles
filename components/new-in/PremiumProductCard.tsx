      ...
      <Link href={`/product/${product.slug}`} className="block">
        <div className="relative aspect-[3/4] bg-[#f5f0eb] overflow-hidden">
          ...
          <button
            ...
            className="absolute top-3 right-3 z-10 p-2 bg-white/90 backdrop-blur-sm rounded-full hover:bg-white transition-all shadow-md hover:scale-110"
          ...
          <div className="absolute inset-0 flex flex-col justify-end opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t from-black/50 via-black/20 to-transparent">
            <div className="flex justify-center gap-2 mb-4">
              ...
              <button
                ...
                className="px-5 py-2.5 bg-black text-white rounded-full hover:bg-gray-800 transition shadow-md hover:scale-105 flex items-center gap-2 text-sm font-medium"
              >
                <ShoppingBag size={16} />
                Add to Cart
              </button>
        ...
        <div className="p-4">
          {product.brand && (
            <p className="text-[10px] font-bold tracking-[0.15em] text-[#D4AF37] uppercase mb-1">{product.brand}</p>
          )}
          <h3 className="text-sm font-medium text-[#1a1a1a] line-clamp-2 mb-1 group-hover:text-[#8b7b6b] transition-colors">{product.title}</h3>
          ...
          <div className="flex items-baseline gap-2 mt-1">
            <span className="text-base font-bold text-[#1a1a1a]">₹{displayPrice.toLocaleString()}</span>
            {originalPrice && (
